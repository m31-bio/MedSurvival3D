...
def main(cfg):
    ...
    base_logger_name = cfg.trainer.logger.get("name", None)
    ...
    for k in range(cfg.data.cv.k):
        ...
        if base_logger_name:
            cfg.trainer.logger.name = f"{base_logger_name}_fold_{cfg.data.module.fold}"
        ...
        csv_logger_dir = cfg.trainer.get("csv_logger_dir", None)
        if csv_logger_dir:
            OmegaConf.set_struct(cfg.trainer, False)
            del cfg.trainer["csv_logger_dir"]
            trainer_logger = cfg.trainer.pop("logger")
            OmegaConf.set_struct(cfg.trainer, True)
            wandb_logger = instantiate(trainer_logger)
            csv_logger = CSVLogger(save_dir=str(csv_logger_dir), name="", version="")
            trainer = instantiate(cfg.trainer, logger=[wandb_logger, csv_logger])
        else:
        ...
        cfg_dict["trainer"].pop("logger", None)
