...
# Dataset
# --------------------------------------------------------------------------- #
class meningioma_T1c_mask_cropped_Data(Dataset):
    """
    T1c + tumor-mask dual-channel meningioma classification dataset.
    ...
    Output of __getitem__:
        img:        torch.Tensor of shape (2, D, H, W) -- ch0 = T1c, ch1 = mask
        label:      int
        subject_id: str
    """
    ...
    def __init__(self, root, split, fold, transform=None,
        ...
        self.case_root = (
            Path(root)
            / "nnsslPlans_onemmiso/Dataset019_UHN_Mayo_T1c_mask/Dataset019_UHN_Mayo_T1c_mask"
        )
...
# DataModule
# --------------------------------------------------------------------------- #
class meningioma_T1c_mask_cropped_DataModule(BaseDataModule):
    ...
    def setup(self, stage: str):
        ...
        self.train_dataset = meningioma_T1c_mask_cropped_Data(
            split="train", transform=self.train_transforms, **ds_kwargs)
        self.val_dataset = meningioma_T1c_mask_cropped_Data(
            split="val", transform=self.test_transforms, **ds_kwargs)
        self.test_dataset = meningioma_T1c_mask_cropped_Data(
