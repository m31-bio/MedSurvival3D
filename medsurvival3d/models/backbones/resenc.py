...
class ResEncoder_Survival(BaseModel):
    def __init__(
        self,
        **hypparams,
    ):
        super(ResEncoder_Survival, self).__init__(**hypparams)

        self.encoder = ResEncoder(**hypparams)

        self.survival_head = SurvivalHead(
            input_dim=320,
            num_time_bins=hypparams["num_time_bins"],
...
def load_pretrained_weights(
    ...
    if in_channels_model != in_channels_pretrained:
        ...
        if (
            "VariableSparkMAETrainer" in pretrained_weights_file
            or "checkpoint_final_S3D" in pretrained_weights_file
            or "/S3D/" in pretrained_weights_file
        ):
